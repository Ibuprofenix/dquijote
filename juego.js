document.addEventListener('DOMContentLoaded', () => {
    ...
    const Juego = {
        ...
        iniciarNivel(n) {
            // SEGURIDAD: Bloqueo de niveles inexistentes
            if (n > 3) {
                console.log("Fin de la gesta. No hay más niveles.");
                this.activo = false;
                if (window.GestionDatos) window.GestionDatos.mostrarTablaRecords();
                return;
            }
            ...
            this.frame = 0; 
            this.bajas = 0;
            if(window.marcador) window.marcador.reset(); 
            this.potenciado = 0;
            this.hDir = 1;
        ...
        loop() {
            ...
            if(window.Interfaz) Interfaz.actualizarDatosDinámicos(marcador.obtener(q.vidas, this.tiempoRestante), this.tiempoRestante);
            ...
            if (s.activo) {
                if (s.estado === 'entrando') { s.x += 4; if (s.x >= 120) { s.estado = 'ayudando'; s.timer = 80; } }
                ...
                else if (s.estado === 'saliendo') { s.x -= 4; if (s.x < -150) s.activo = false; }
    ...
    const q = this.entidades.quijote;
    const resultados = window.marcador.obtener(q.vidas, this.tiempoRestante);
    ...
    if (victoria) {
        if(window.GestionDatos) window.GestionDatos.enviarGesta(resultados.total, q.vidas, this.nivel);

        if (btnSiguiente) {
            // 1. Limpiamos y clonamos el botón SIEMPRE que haya victoria
            const nuevoBtn = btnSiguiente.cloneNode(true);
            btnSiguiente.parentNode.replaceChild(nuevoBtn, btnSiguiente);
            
            // 2. Lo hacemos visible de inmediato
            nuevoBtn.classList.remove('hidden');
            nuevoBtn.style.display = 'block';

            if (this.nivel < 3) {
                nuevoBtn.innerText = "SIGUIENTE NIVEL";
                nuevoBtn.onclick = () => {
                    window.Trivia.lanzar(`fase${this.nivel}`, () => {
                        this.iniciarNivel(this.nivel + 1);
                    });
                };
            } else {
                // NIVEL 3: El botón debe decir esto y lanzar la última trivia
                nuevoBtn.innerText = "EL ÚLTIMO DESAFÍO";
                nuevoBtn.onclick = () => {
                    window.Trivia.lanzar('fase3', () => {
                        if (tituloFinal) tituloFinal.innerText = "¡LIBERTADOR DE LA MANCHA!";
                        nuevoBtn.style.display = 'none'; 
                        nuevoBtn.classList.add('hidden');
                        if (window.GestionDatos) window.GestionDatos.mostrarTablaRecords();
                    });
                };
            }
        ...
        // Lógica de derrota
        if (tituloFinal) tituloFinal.innerText = "¡HAS CAÍDO EN COMBATE!";
        if (btnSiguiente) {
            btnSiguiente.classList.remove('hidden');
            btnSiguiente.style.display = 'block';
            btnSiguiente.innerText = "REINTENTAR";
            btnSiguiente.onclick = () => this.iniciarNivel(this.nivel);
        }
    }

    // IMPORTANTE: Llamar a la interfaz al final para que no oculte los botones recién configurados
    if (window.Interfaz) Interfaz.mostrarPantallaFinal(victoria, resultados);
}
    };
    ...
    if(btnComenzar) {
        btnComenzar.onclick = () => {
            ...
            if(window.GestionDatos) GestionDatos.setNombre(nombre || "HIDALGO");
            document.getElementById('registro-caballero').classList.add('hidden');
            
            if(window.Trivia) {
                const tCont = document.getElementById('trivia-container');
                if(tCont) {
