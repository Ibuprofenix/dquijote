document.addEventListener('DOMContentLoaded', () => {
    ...
    const Juego = {
        ...
        puntos: 0, tiempoRestante: 90, ultimoSegundo: 0, 
            ...
            this.entidades.lanzas.forEach((l, i) => {
                ...
                if(this.entidades.boss && l.x > this.entidades.boss.x && l.x < this.entidades.boss.x+this.entidades.boss.w && l.y < this.entidades.boss.y+this.entidades.boss.h) {
                    ...
                    if(this.entidades.boss.hp <= 0) { 
                        ...
                        ctx.fillStyle = "white"; ctx.font = "bold 40px 'Cinzel'"; ctx.textAlign = "center";
                        ctx.fillText("COLOSO DERROTADO", 400, 300);
                    }
                }
        ...
        fin(victoria, tituloForzado = null) { 
            this.activo = false;
            let tituloGesta = "";

            if (victoria) {
                // Si es victoria absoluta (se llama tras la última trivia)
                tituloGesta = "DESFACEDOR DE ENTUERTOS";
            } else {
                // Si es derrota, usamos el título forzado de la trivia o el del nivel actual
                const nombresNiveles = [
                    "EL ESPALDARAZO",           // Nivel 1
                    "EL YELMO DE MAMBRINO",     // Nivel 2
                    "EL BÁLSAMO DE FIERABRÁS",  // Nivel 3
                    "LA CUEVA DE MONTESINOS"    // Trivia Final
                ];
                tituloGesta = tituloForzado || nombresNiveles[this.nivel - 1] || "EL ESPALDARAZO";
            }

            if(window.AudioEngine) AudioEngine.reproducir(victoria ? 'victoria' : 'muerte');
            
            // Pasar el título a la interfaz para mostrarlo en el cartel
            if(window.Interfaz) Interfaz.mostrarPantallaFinal(victoria, this.puntos, tituloGesta);
            ...
            if (victoria) {
                if(btnRanking) btnRanking.classList.add('hidden');
                if(btnSiguiente) {
                    btnSiguiente.classList.remove('hidden');
                    const textosBotones = ["EL YELMO DE MAMBRINO", "EL BÁLSAMO DE FIERABRÁS", "LA CUEVA DE MONTESINOS"];
                    btnSiguiente.innerText = textosBotones[this.nivel - 1] || "SIGUIENTE GESTA";
                    
                    btnSiguiente.onclick = () => {
                        document.getElementById('pantalla-final').classList.add('hidden');
                        if (this.nivel < 3) {
                            if (typeof Trivia !== 'undefined') {
                                Trivia.lanzar(`fase${this.nivel}`, () => this.iniciarNivel(this.nivel + 1));
                            } else {
                            ...
                            if (typeof Trivia !== 'undefined') {
                                Trivia.lanzar('fase3', () => {
                                    // Al completar la última trivia con éxito
                                    this.fin(true); // Llamada final para victoria total
                                });
                            } else {
                                if(window.GestionDatos) GestionDatos.guardarYMostrarRanking(this.puntos, true, "DESFACEDOR DE ENTUERTOS");
                            }
                        }
                    };
                }
                ...
                if(btnRanking) {
                    ...
                    btnRanking.onclick = () => {
                        document.getElementById('pantalla-final').classList.add('hidden');
                        if(window.GestionDatos) GestionDatos.guardarYMostrarRanking(this.puntos, false, tituloGesta);
                    };
                }
            }
        }
    ...
    if(btnComenzar) {
        btnComenzar.innerText = "EL ESPALDARAZO";
        btnComenzar.onclick = () => {
            ...
            if (typeof Trivia !== 'undefined') {
                Trivia.lanzar('fase0', () => Juego.iniciarNivel(1));
            } else {
