/**
 * MOTOR DE JUEGO: DON QUIJOTE VS GIGANTES
 */
const AMBIENTES = {
    "index": { sky: "linear-gradient(to bottom, #4facfe, #f5deb3)" },
    "Nivel1": { sky: "linear-gradient(to bottom, #4facfe, #f5deb3)" }
};

let canvas, ctx;
let quijote = { x: 400, y: 530, vidas: 3 };
let enemigos = [], proyectiles = [], lanzas = [], trizas = [], activo = false;
let hDir = 1, velHorda = 0.8, tiempoRestante = 120, timerInterval;

const imgQ = new Image(); imgQ.src = 'sprites_quijote.png';
const imgM = new Image(); imgM.src = 'sprites_molino.png';
const imgA = new Image(); imgA.src = 'sprites_aspas.png';
const imgF = new Image(); imgF.src = 'sprites_rafaga.png';

function playSfx(tipo) {
    const actx = new (window.AudioContext || window.webkitAudioContext)();
    const osc = actx.createOscillator();
    const g = actx.createGain();
    osc.connect(g); g.connect(actx.destination);
    osc.start(); osc.stop(actx.currentTime + 0.1);
}

function posicionarEnemigos() {
    enemigos = [];
    for(let f=0; f<3; f++) {
        for(let c=0; c<8; c++) {
            enemigos.push({ x: 100 + (c * 85), y: 80 + (f * 100), rot: Math.random()*Math.PI, cargando: false, timer: 0 });
        }
    }
}

function iniciarJuego() {
    Interfaz.puntuacion = 0;
    tiempoRestante = 120;
    quijote.vidas = 3;
    activo = true;
    posicionarEnemigos();
    if(timerInterval) clearInterval(timerInterval);
    timerInterval = setInterval(() => {
        if(activo) {
            tiempoRestante--;
            if(tiempoRestante <= 0) finalizar(false, "El tiempo expiró.");
        }
    }, 1000);
}

function finalizar(victoria, msg) {
    activo = false;
    clearInterval(timerInterval);
    Interfaz.mostrarMenuFinal(victoria ? "VICTORIA" : "DERROTA", msg, victoria, "Nivel2", { vidas: quijote.vidas, tiempo: tiempoRestante });
}

function loop() {
    if (!ctx) return requestAnimationFrame(loop);
    ctx.clearRect(0, 0, 800, 600);

    // Dibujar enemigos siempre para que no se vea vacío
    enemigos.forEach(e => {
        if (activo) e.rot += 0.04;
        ctx.drawImage(imgM, e.x - 45, e.y - 45, 90, 90);
        ctx.save();
        ctx.translate(e.x, e.y - 10); ctx.rotate(e.rot);
        ctx.drawImage(imgA, -50, -50, 100, 100);
        ctx.restore();
    });

    if (activo) {
        // Lógica de horda
        let bajar = false;
        enemigos.forEach(e => {
            e.x += velHorda * hDir;
            if (e.x > 760 || e.x < 40) bajar = true;
            if (Math.random() < 0.005 && !e.cargando) e.cargando = true;
            if (e.cargando && ++e.timer > 40) {
                proyectiles.push({ x: e.x, y: e.y, s: 20 });
                e.cargando = false; e.timer = 0;
            }
        });
        if (bajar) { hDir *= -1; enemigos.forEach(e => e.y += 20); }

        // Quijote
        if (teclado['ArrowLeft']) quijote.x -= 7;
        if (teclado['ArrowRight']) quijote.x += 7;
        quijote.x = Math.max(40, Math.min(760, quijote.x));
        ctx.drawImage(imgQ, 480, 0, 480, 440, quijote.x - 50, quijote.y - 45, 100, 92);

        // Lanzas y colisiones
        lanzas.forEach((l, li) => {
            l.y -= 10;
            ctx.fillStyle = "black"; ctx.fillRect(l.x, l.y, 2, 15);
            enemigos.forEach((e, ei) => {
                if (Math.abs(l.x - e.x) < 40 && Math.abs(l.y - e.y) < 40) {
                    enemigos.splice(ei, 1); lanzas.splice(li, 1);
                    Interfaz.añadirPuntos(100);
                }
            });
        });
        
        if (enemigos.length === 0) finalizar(true, "¡Molinos vencidos!");
        if (quijote.vidas <= 0) finalizar(false, "Has caído.");
    } else {
        ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(0,0,800,600);
        ctx.fillStyle = "white"; ctx.textAlign = "center"; ctx.font = "30px Almendra";
        ctx.fillText("PULSA ESPACIO PARA LUCHAR", 400, 300);
    }
    
    // UI
    ctx.fillStyle = "#f1c40f"; ctx.textAlign = "left"; ctx.font = "20px MedievalSharp";
    ctx.fillText(`VIDAS: ${quijote.vidas} | TIEMPO: ${tiempoRestante}`, 20, 40);

    requestAnimationFrame(loop);
}

const teclado = {};
window.onkeydown = (e) => {
    teclado[e.key] = true;
    if (e.key === " ") {
        if (!activo) iniciarJuego();
        else lanzas.push({ x: quijote.x, y: quijote.y });
    }
};
window.onkeyup = (e) => teclado[e.key] = false;

window.onload = () => {
    canvas = document.getElementById('gameCanvas');
    ctx = canvas.getContext('2d');
    const path = window.location.pathname.split("/").pop().replace(".html", "") || "index";
    if (AMBIENTES[path]) canvas.style.background = AMBIENTES[path].sky;
    posicionarEnemigos();
    loop();
};
